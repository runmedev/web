...
const RUNME_SHARED_NOTEBOOK_INSTRUCTIONS = [
  ...
  'When you need notebook API details, inspect the runtime contract with await help(), await notebooks.help(), or await notebooks.help("createLocal" | "appendCodeCell" | "appendMarkdownCell" | "update" | "get" | "execute").',
  'Prefer the high-level helpers when they fit: await notebooks.createLocal("helloworld"), await notebooks.appendCodeCell({ value: \'print("hello world")\', languageId: "python" }), await notebooks.appendMarkdownCell({ value: "# Notes" }), and runmeRunners.ensure("openai-local", "ws://localhost:9988/ws", { setDefault: true }).',
