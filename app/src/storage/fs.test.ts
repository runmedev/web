  ...
  g.Blob = class Blob {
    ...
    get size() {
      return this.parts.reduce(
        (acc: number, p: any) =>
          acc + (typeof p === 'string' ? Buffer.byteLength(p, 'utf8') : 0),
        0
      )
    }
  }
...
describe('URI helpers (via FilesystemNotebookStore)', () => {
  it('getType returns File for file URIs', async () => {
    ...
    const type = await store.getType('fs://workspace/abc123/file/notebook.json')
...
describe('FilesystemNotebookStore', () => {
  ...
  describe('list', () => {
    it('returns .json files and directories', async () => {
      ...
      rootEntries.set(
        'notebook.json',
        createMockFileHandle('notebook.json', nbJson)
      )
      rootEntries.set('readme.md', createMockFileHandle('readme.md', '# Hello'))
      rootEntries.set('sub', createMockDirectoryHandle('sub', new Map()))
    ...
    it('builds correct URIs for files', async () => {
      rootEntries.set(
        'test.json',
        createMockFileHandle('test.json', makeEmptyNotebookJson())
      )
  ...
  describe('load', () => {
    ...
    it('throws when given a directory URI', async () => {
      await expect(store.load(ROOT_URI)).rejects.toThrow('expects a file URI')
    })
  ...
  describe('create', () => {
    ...
    it('refuses to overwrite an existing notebook', async () => {
      ...
      await expect(
        existing.getFile().then((file) => file.text())
    ...
    it('serializes concurrent creates through overlapping workspace roots', async () => {
      ...
      ;(rootHandle as any).resolve = vi.fn(async (other: FileSystemHandle) =>
      ...
      const results = await Promise.allSettled([
        store.createContent(outerSubdirectoryUri, 'shared.runme', firstContent),
    ...
    it('does not double .json extension', async () => {
      ...
      expect(rootHandle.getFileHandle).toHaveBeenCalledWith('already.json', {
        create: true,
      })
    })
  ...
  describe('requestPermission', () => {
    ...
    it('throws for unknown workspace', async () => {
      await expect(store.requestPermission('nonexistent')).rejects.toThrow(
        'Workspace not found'
      )
    })
  })
  ...
  describe('handle resolution', () => {
    ...
    it('throws for nonexistent workspace', async () => {
      ...
      await expect(store.load(fileUri)).rejects.toThrow('Workspace not found')
    })
  })
})
