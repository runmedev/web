...
describe('CurrentDocInitializer', () => {
  ...
  it('keeps doc params when opening the notebook fails', async () => {
    ...
    mocks.openNotebook.mockRejectedValue(
      new Error('Notebook store is not ready')
    )
  ...
  it('opens the Drive sync status document from a doc param', async () => {
    ...
    await waitFor(() => {
      expect(mocks.setCurrentDoc).toHaveBeenCalledWith('status://drive-sync')
    })
    expect(mocks.openNotebook).not.toHaveBeenCalled()
    expect(mocks.showDocument).toHaveBeenCalledWith('status://drive-sync', {
