...
const currentDocMocks = vi.hoisted(() => ({
  currentDoc: null as string | null,
  lastNotebookDoc: null as string | null,
  openNotebooks: [] as Array<{ uri: string; name?: string }>,
  notebookDataByUri: new Map<string, unknown>(),
  setCurrentDoc: vi.fn(),
}))

const runmeConsoleMocks = vi.hoisted(() => ({
  createRunmeConsoleApi: vi.fn(
    ({
      resolveNotebook,
    }: {
      resolveNotebook: (target?: unknown) => unknown
    }) => ({ resolveNotebook })
  ),
}))
...
vi.mock('../../contexts/CurrentDocContext', () => ({
  useCurrentDoc: () => ({
    getCurrentDoc: () => currentDocMocks.currentDoc,
    getLastNotebookDoc: () => currentDocMocks.lastNotebookDoc,
    setCurrentDoc: currentDocMocks.setCurrentDoc,
  }),
}))

vi.mock('../../contexts/NotebookContext', () => ({
  useNotebookContext: () => ({
    getNotebookData: (uri: string) =>
      currentDocMocks.notebookDataByUri.get(uri) ?? null,
    openNotebook: vi.fn(),
    useNotebookList: () => currentDocMocks.openNotebooks,
  }),
}))
...
vi.mock('../../lib/runtime/runmeConsole', () => ({
  createRunmeConsoleApi: runmeConsoleMocks.createRunmeConsoleApi,
}))
...
vi.mock('../Actions/ActionOutputItems', () => ({
  ActionOutputItems: ({
    outputs,
  }: {
    ...
    return (
      <textarea
        ...
        onKeyDown={(event) => {
          ...
          const handler =
            code && modifier !== null
              ? commandsRef.current.get(modifier | code)
              : undefined
          if (!handler) {
            return
          }
...
import { APP_CONSOLE_DOCUMENT_URI } from '../../lib/workspaceDocuments/workspaceDocumentTypes'
...
describe('AppConsole', () => {
  beforeEach(() => {
    ...
    currentDocMocks.currentDoc = null
    currentDocMocks.lastNotebookDoc = null
    currentDocMocks.openNotebooks = []
    currentDocMocks.notebookDataByUri.clear()
    currentDocMocks.setCurrentDoc.mockClear()
    runmeConsoleMocks.createRunmeConsoleApi.mockClear()
  })
  ...
  it('preserves the in-progress draft while browsing history', async () => {
    ...
    fireEvent.keyDown(currentInput(), { key: 'Enter', shiftKey: true })
    await waitFor(() =>
      expect(screen.getAllByTestId('app-console-cell')).toHaveLength(3)
    )
  ...
  it('renders history controls with tooltips', async () => {
    ...
    await screen.findByLabelText('App Console input')
    expect(
      screen.getByTestId('app-console-history-previous').getAttribute('title')
    ).toBe('Previous history entry (Alt+P)')
    expect(
      screen.getByTestId('app-console-history-next').getAttribute('title')
    ).toBe('Next history entry (Alt+N)')
  })
  ...
  it('resolves the previously selected notebook when the app console tab is active', async () => {
    const notebookUri = 'local://file/previous-notebook'
    const notebookData = {
      getUri: vi.fn(() => notebookUri),
      getName: vi.fn(() => 'previous-notebook.runme.md'),
      getNotebook: vi.fn(() => ({ cells: [], metadata: {} })),
      updateCell: vi.fn(),
      getCell: vi.fn(),
    }
    currentDocMocks.currentDoc = APP_CONSOLE_DOCUMENT_URI
    currentDocMocks.lastNotebookDoc = notebookUri
    currentDocMocks.openNotebooks = [{ uri: 'local://file/other-notebook' }]
    currentDocMocks.notebookDataByUri.set(notebookUri, notebookData)

    render(<AppConsole showHeader={false} />)

    await screen.findByLabelText('App Console input')
    const createCall = runmeConsoleMocks.createRunmeConsoleApi.mock.calls.at(-1)
    const resolveNotebook = createCall?.[0]?.resolveNotebook

    expect(resolveNotebook?.()).toBe(notebookData)
  })
