...
> Follow-up: the long-running operation lifecycle, timeout semantics, polling,
> cancellation, idempotency, and output persistence are specified in the
> [ExecuteCode long-running operations design](https://runme.gateway.unified-0.internal.api.openai.org/?doc=https%3A%2F%2Fdrive.google.com%2Ffile%2Fd%2F1txTgo15Ugl_Gtg-hTNg0OONAz_PT-OnF%2Fview).
> That design preserves a single start API while adding
> `GetExecuteCodeOperation` and `CancelExecuteCodeOperation` lifecycle APIs.

## Status

Historical v0 proposal. The follow-up design above supersedes the WebMCP
response lifecycle and output contract. The AppKernel execution model remains
current.
